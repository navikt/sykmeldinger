...
const CheckboxEntry: React.FC<CheckboxEntryProps> = ({ show, checkboxText, margin = true }) => {
    if (show) {
        return (
            <div className="checkbox-entry" style={{ marginBottom: margin ? '2rem' : '' }}>
                {Array.isArray(checkboxText) ? (
                    checkboxText.map((str, index) => (
                        <div
                            key={index}
                            className="checkbox-entry__checbox"
                            style={{ display: 'flex', alignItems: 'center', marginBlock: '1rem' }}
                        >
                            <CheckboxSvg />
                            <Normaltekst style={{ marginLeft: '0.5rem' }}>{str}</Normaltekst>
                        </div>
                    ))
                ) : (
                    <div className="checkbox-entry__checbox" style={{ display: 'flex', alignItems: 'center' }}>
                        <CheckboxSvg />
                        <Normaltekst style={{ marginLeft: '0.5rem' }}>{checkboxText}</Normaltekst>
                    </div>
                )}
            </div>
        );
