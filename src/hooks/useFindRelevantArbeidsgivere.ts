import { isWithinInterval, isSameDay, subDays, isBefore } from 'date-fns'
...
interface PossibleArbeidsgiver {
...
export function useFindRelevantArbeidsgivere(
    ...
    const arbeidsgivereCombined: PossibleArbeidsgiver[] = R.pipe(
        ...
        R.uniqBy((x: PossibleArbeidsgiver) => x.orgnummer),
    )
...
function getSykmeldingerEdgeToEdge(sykmelding: SykmeldingFragment, others: SykmeldingFragment[]): SykmeldingFragment[] {
    ...
    if (isWithinInterval(sykmeldingFom, behindInterval) || isSameDay(subDays(sykmeldingFom, 1), behindTom)) {
