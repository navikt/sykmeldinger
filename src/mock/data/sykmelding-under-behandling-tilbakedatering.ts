import { formatISO, parseISO, sub, add } from 'date-fns';

export const sykmeldingUnderbehandlingTilbakedatering = (mottatt = formatISO(sub(new Date(), { months: 11 }))) => {
    const addDays = (date: string, days: number) => formatISO(add(parseISO(date), { days }));
    ...
    return {
        ...
        sykmeldingsperioder: [
            {
                fom: addDays(mottatt, 5),
                tom: addDays(mottatt, 17),
                gradert: null,
        ...
        behandletTidspunkt: addDays(mottatt, 2),
        behandler: {
            fornavn: 'Frida',
            mellomnavn: 'Perma',
            etternavn: 'Frost',
